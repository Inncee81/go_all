...
func main() {
	...
	isNotFound := db.Where("username = ?", "tizi366").First(&u).RecordNotFound()
	if isNotFound {
		fmt.Println("找不到记录")
		return
	}
	//打印查询到的数据
	fmt.Println(u.Username, u.Password, u.ID)

	//-----------查询并返回所有数据
	//因为Find返回的是数组，所以定义一个商品数组用来接收结果
	var us []User
	//等价于：SELECT * FROM `foods`
	isFound := db.Where("username=?", "tizi366").Find(&us).RecordNotFound()
	if isFound {
		fmt.Println("找不到记录")
		return
	}
